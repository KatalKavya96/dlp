...
test("server-renders the finished Dharohar brand gateway", async () => {
  ...
  assert.match(html, /The collection travels/);
  assert.match(html, /beyond the flame\./);
...
test("keeps the production shell, SEO, and accessibility safeguards in source", async () => {
  ...
  assert.ok(gateway.indexOf("<PersonalisationStudio />") < gateway.indexOf("<RitualUniverse />"));
  assert.ok(gateway.indexOf("<TrustSequence />") < gateway.indexOf("<RitualUniverse />"));
  ...
  assert.match(gateway, /function EditorialCollection/);
