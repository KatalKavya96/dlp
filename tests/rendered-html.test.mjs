...
test("server-renders the finished Dharohar brand gateway", async () => {
  ...
  assert.match(html, /The collection travels beyond the flame\./);
  ...
  assert.match(html, /Made to return\./);
  assert.match(html, /Never made to be replaced\./);
...
test("keeps the production shell, SEO, and accessibility safeguards in source", async () => {
  ...
  await access(new URL("../public/images/curated/ptal-copper-tumbler.jpg", import.meta.url));
  await access(new URL("../public/images/curated/ptal-copper-dispenser-lifestyle.jpg", import.meta.url));
  await access(new URL("../public/images/curated/ptal-brass-davara-lifestyle.jpg", import.meta.url));
});
