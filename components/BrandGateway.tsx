    ...
    <section ref={heroRef} onMouseMove={pointerMove} id="top" className="cinematic-hero relative isolate min-h-[calc(100svh-108px)] overflow-hidden bg-[#17110d] text-[#fff5df]" aria-labelledby="hero-title">
      <AnimatePresence initial={false} mode="sync">
        <motion.div key={heroFrames[frame].image} className="absolute inset-0 -z-30" initial={{ opacity: 0, scale: 1.035 }} animate={{ opacity: 1, scale: playing ? 1.085 : 1.04 }} exit={{ opacity: 0 }} transition={{ opacity: { duration: 1.8 }, scale: { duration: 9, ease: "linear" } }}>
          <Image src={heroFrames[frame].image} alt="Copper and brass heritage cookware and its making" fill priority={frame === 0} unoptimized sizes="100vw" className="object-cover" style={{ objectPosition: heroFrames[frame].position }} />
        </motion.div>
      </AnimatePresence>
...
function GenerationalStory() {
  return (
    <section id="journey" className="bg-[#17100c] px-5 py-[clamp(6rem,10vw,10rem)] text-[#fff4dc]" aria-labelledby="journey-title">
      <div className="site-container">
        ...
        <div className="mt-16 grid gap-5 lg:grid-cols-3">{generations.map((item, index) => <motion.article key={item.number} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: .85, delay: index * .14 }} className="cinematic-portal group relative min-h-[620px] overflow-hidden rounded-[1.8rem]"><Image src={item.image} alt="" fill unoptimized sizes="(max-width: 1024px) 100vw, 33vw" className="object-cover transition duration-[1400ms] ease-out group-hover:scale-[1.06]" /><div className="absolute inset-0 bg-gradient-to-t from-[#100a07] via-[#100a07]/18 to-black/5" /><div className="absolute inset-x-0 bottom-0 p-7 sm:p-9"><span className="portal-number font-serif text-7xl text-[#e6c77f]/35">{item.number}</span><p className="mt-5 text-[9px] font-bold uppercase tracking-[.22em] text-[#e2c27d]">{item.era}</p><h3 className="mt-3 font-serif text-4xl leading-[.96]">{item.title}</h3><p className="mt-4 text-sm leading-7 text-white/58">{item.copy}</p></div></motion.article>)}</div>
      </div>
    </section>
  );
}
...
function GuidedFinder() {
  ...
  const result = useMemo(() => {
    if (answers.moment.includes("wedding") || answers.cooking.includes("gift")) return { title: "A personalised celebration piece", copy: "Begin with copper or brass forms that allow engraving and presentation-ready gifting.", path: "/collections/wedding-gifts" };
    if (answers.moment.includes("complete") || answers.household.includes("large")) return { title: "A considered complete rasoi", copy: "Build a practical family set across sizes with a consultation before purchase.", path: "/collections/complete-rasoi" };
    if (answers.moment.includes("design") || answers.household.includes("hospitality")) return { title: "A project-led collection", copy: "Start with a materials and volume conversation for spaces, tables and hospitality.", path: "/pages/trade-and-hospitality" };
    return { title: "One useful everyday heirloom", copy: "Start with a versatile brass kadhai or patila chosen for the food you make most.", path: "/collections/everyday-cookware" };
  }, [answers]);
  ...
  return (
    <section id="finder" className="bg-[#2a1b13] px-5 py-[clamp(5rem,9vw,8rem)] text-[#fff3da]" aria-labelledby="finder-title">
      <div className="site-container grid gap-10 lg:grid-cols-[.85fr_1.15fr] lg:items-center">
        <Reveal><p className="text-[10px] font-bold uppercase tracking-[.24em] text-[#d8b86b]">A quiet recommendation</p><h2 id="finder-title" className="mt-6 font-serif text-[clamp(3.6rem,6vw,6.7rem)] leading-[.88]">Find your first Dharohar piece.</h2><p className="mt-7 max-w-xl text-base leading-8 text-white/58">Three simple answers create a more useful doorway into the commerce collection.</p><div className="mt-8 flex gap-2">{questions.map((question, index) => <span key={question.key} className={`h-1.5 rounded-full transition-all ${index <= step ? "w-10 bg-[#d8b86b]" : "w-5 bg-white/15"}`} />)}</div></Reveal>
        <div className="min-h-[430px] rounded-[2rem] border border-white/12 bg-white/[.055] p-6 backdrop-blur sm:p-10">
          <AnimatePresence mode="wait">{step < questions.length ? <motion.div key={step} initial={{ opacity: 0, x: 32 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -32 }} transition={{ duration: .45 }}><p className="text-[9px] font-bold uppercase tracking-[.2em] text-[#d8b86b]">Question {step + 1} of 3</p><h3 className="mt-5 font-serif text-4xl sm:text-5xl">{questions[step].prompt}</h3><div className="mt-8 grid gap-3 sm:grid-cols-2">{questions[step].options.map((option) => <button key={option} type="button" onClick={() => choose(option)} className="group flex min-h-24 items-center justify-between rounded-2xl border border-white/14 bg-black/10 p-5 text-left text-sm text-white/75 transition hover:border-[#d8b86b]/65 hover:bg-[#d8b86b]/10 hover:text-white"><span>{option}</span><ArrowRight size={16} className="text-[#d8b86b] transition group-hover:translate-x-1" /></button>)}</div></motion.div> : <motion.div key="result" initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }} className="flex min-h-[350px] flex-col justify-center"><WandSparkles className="text-[#d8b86b]" size={32} /><p className="mt-6 text-[9px] font-bold uppercase tracking-[.2em] text-[#d8b86b]">Your considered beginning</p><h3 className="mt-4 max-w-xl font-serif text-5xl sm:text-6xl">{result.title}</h3><p className="mt-5 max-w-xl text-base leading-8 text-white/58">{result.copy}</p><div className="mt-8 flex flex-wrap gap-3"><StoreLink path={result.path} eventLabel="finder_result" className="heritage-button heritage-button-filled">Continue with this route <ArrowRight size={15} /></StoreLink><button type="button" onClick={() => { setStep(0); setAnswers({ moment: "", cooking: "", household: "" }); }} className="heritage-button !border-white/20 !text-white"><RotateCcw size={14} /> Begin again</button></div></motion.div>}</AnimatePresence>
        </div>
      </div>
    </section>
  );
...
function ConsultationGateway() {
  ...
  return <form onSubmit={submit} className="rounded-[2rem] border border-[#e3c27c]/28 bg-white/[.07] p-6 backdrop-blur sm:p-8"><div className="grid gap-4 sm:grid-cols-2"><label><span className="consult-label">Your name</span><input required name="name" className="consult-input" placeholder="Name" /></label><label><span className="consult-label">Email</span><input required name="email" type="email" className="consult-input" placeholder="you@example.com" /></label></div><div className="mt-4 grid gap-4 sm:grid-cols-2"><label><span className="consult-label">I am interested in</span><select name="interest" className="consult-input bg-[#4e3524]" value={interest} onChange={(event) => setInterest(event.target.value)}><option>Complete heritage kitchen</option><option>Wedding or festive gifting</option><option>Everyday cookware</option><option>Design or hospitality project</option><option>Engraving consultation</option></select></label><label><span className="consult-label">Preferred time</span><select name="timing" className="consult-input bg-[#4e3524]" defaultValue="Weekday morning"><option>Weekday morning</option><option>Weekday afternoon</option><option>Weekday evening</option><option>Weekend</option></select></label></div><AnimatePresence initial={false}>{gifting ? <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden"><div className="mt-4 grid gap-4 sm:grid-cols-3"><label><span className="consult-label">Occasion date</span><input name="occasionDate" type="date" className="consult-input [color-scheme:dark]" /></label><label><span className="consult-label">Quantity</span><input name="quantity" inputMode="numeric" className="consult-input" placeholder="e.g. 25" /></label><label><span className="consult-label">Budget range</span><input name="budget" className="consult-input" placeholder="₹ range" /></label></div></motion.div> : null}</AnimatePresence><label className="mt-4 block"><span className="consult-label">Tell us a little more</span><textarea name="message" rows={4} className="consult-input h-auto resize-none py-3" placeholder="The occasion, family size, preferred metals or engraving needs…" /></label><button type="submit" className="mt-4 inline-flex min-h-13 w-full items-center justify-center gap-3 rounded-xl bg-[#d3a84d] px-6 text-[10px] font-bold uppercase tracking-[.15em] text-[#3c291b] transition hover:bg-[#e2c275]">Prepare consultation request <Send size={15} /></button><p aria-live="polite" className="mt-3 min-h-5 text-center text-xs text-[#e8cc91]">{status}</p></form>;
}
  ...
  return (
    <>
      ...
      <main>
        ...
        <OccasionRoutes />
        <TrustSequence />
        <section id="consultation" className="bg-[#4d3421] px-5 text-[#fff8e9]" aria-labelledby="consultation-title"><div className="site-container grid gap-12 py-[clamp(5rem,9vw,8rem)] lg:grid-cols-[1fr_1.05fr] lg:items-center"><Reveal><p className="text-[10px] font-bold uppercase tracking-[.24em] text-[#e0bd76]">Gift concierge · Kitchen consultation</p><h2 id="consultation-title" className="mt-5 max-w-2xl font-serif text-[clamp(3.8rem,6vw,6.8rem)] leading-[.88]">Build a kitchen your family will remember.</h2><p className="mt-6 max-w-xl text-base leading-8 text-white/65">Tell us about the rituals, gifting moment or design project you are considering. We’ll prepare a thoughtful route into the collection.</p><div className="mt-8 flex flex-wrap gap-3">{booking ? <a href={booking} target="_blank" rel="noreferrer" onClick={() => track("booking_click", "consultation")} className="heritage-button heritage-button-filled"><CalendarDays size={15} /> Reserve 20 minutes</a> : <a href={`mailto:${process.env.NEXT_PUBLIC_CONSULTATION_EMAIL ?? "hello@dharohar.in"}?subject=Dharohar%2020-minute%20consultation`} className="heritage-button heritage-button-filled"><Clock3 size={15} /> Request 20 minutes</a>}<a href={`mailto:${process.env.NEXT_PUBLIC_CONSULTATION_EMAIL ?? "hello@dharohar.in"}`} className="heritage-button !border-white/25 !text-white"><Mail size={15} /> Email directly</a></div><div className="mt-10 flex flex-wrap gap-5 text-xs text-white/55"><span className="flex items-center gap-2"><Users size={15} className="text-[#e0bd76]" /> Personal recommendations</span><span className="flex items-center gap-2"><Gift size={15} className="text-[#e0bd76]" /> Gifting and engraving</span></div></Reveal><ConsultationGateway /></div></section>
      </main>
